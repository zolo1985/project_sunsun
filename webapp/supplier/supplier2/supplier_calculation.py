...
supplier2_calculation_blueprint = Blueprint('supplier2_calculation', __name__)


@supplier2_calculation_blueprint.route('/supplier2/calculation/two-weeks', methods=['GET','POST'])
@login_required
@has_role('supplier2')
def supplier2_calculation_two_week():
    ...
    if form.select_date.data is not None and form.validate_on_submit():
        ...
        supplier_format.insert(4, (daily_data))
        suppliers_datas.append(supplier_format)
        return render_template('/supplier/supplier2/supplier_calculation_two_week.html', suppliers_datas=suppliers_datas, current_date=current_date, day_list=days_data, form=form)
