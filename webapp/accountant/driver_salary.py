...
def accountant_driver_salary():
    ...
    for driver in drivers:
        ...
        days_list = [
            (i.day,
            connection.execute("SELECT COUNT(delivery.id) as total_delivery_count FROM sunsundatabase2.user as driver join sunsundatabase2.delivery as delivery on driver.id=delivery.assigned_driver_id where (date(delivery.delivered_date)=:date0) and driver.id=:driver_id and delivery.is_delivered=true group by driver.id", {"date0":i.date(), "driver_id": driver.id},).scalar() or 0,
            connection.execute("SELECT COUNT(*) as total FROM sunsundatabase2.pickup_task as pickup WHERE (pickup.clerk_received_date BETWEEN :date1 AND :date2) and pickup.assigned_driver_id=:driver_id and pickup.status='completed';", {"date1": (i - timedelta(days=1)).replace(hour=17, minute=0), "date2": i.replace(hour=17, minute=0), "driver_id": driver.id},).scalar() or 0,
            connection.execute("SELECT COUNT(*) as total FROM sunsundatabase2.return_task as return_task WHERE (return_task.supplier_received_date BETWEEN :date1 AND :date2) and return_task.assigned_driver_id=:driver_id and return_task.is_completed=true;", {"date1": (i - timedelta(days=1)).replace(hour=17, minute=0), "date2": i.replace(hour=17, minute=0), "driver_id": driver.id},).scalar() or 0,)
