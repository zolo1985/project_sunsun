...
@accountant_supplier_calculation_blueprint.route('/accountant/supplier/calculation/daily', methods=['GET','POST'])
@login_required
@has_role('accountant')
def accountant_supplier_calculation_daily():
    ...
    if form.validate_on_submit():
        ...
        suppliers_delivery_total = connection.execute("SELECT supplier.company_name as supplier_name, GROUP_CONCAT(delivery.id) as delivery_ids, count(delivery.id) as total_delivery_count, sum(delivery.total_amount) as total_amount, supplier.is_invoiced as is_invoiced, supplier.supplier_rate as fee FROM sunsundatabase2.user as supplier join sunsundatabase2.delivery as delivery on supplier.id=delivery.user_id where delivery.received_from_clerk_date >= :yesterday and delivery.received_from_clerk_date < :selected_date and delivery.is_delivered=true group by supplier.id;", {"yesterday": selected_date_time_minus_1day, "selected_date": selected_date_add_time_replace}).all()
        suppliers_warehouse_sales_total = connection.execute("SELECT supplier.company_name as supplier_name, GROUP_CONCAT(warehouse_sale.id) as warehouse_sale_ids, count(warehouse_sale.id) as total_warehouse_sale_count, CAST(sum(warehouse_sale.total_amount) AS SIGNED) as total_amount, supplier.is_invoiced as is_invoiced, 0 as fee FROM sunsundatabase2.user as supplier join sunsundatabase2.warehouse_sale as warehouse_sale on supplier.id=warehouse_sale.supplier_id where warehouse_sale.created_date >= :yesterday and warehouse_sale.created_date < :selected_date and warehouse_sale.is_completed=true group by supplier.id;", {"yesterday": selected_date_time_minus_1day, "selected_date": selected_date_add_time_replace}).all()
        return render_template('/accountant/supplier_calculation_daily.html', form=form, suppliers_delivery_total=suppliers_delivery_total, suppliers_warehouse_sales_total=suppliers_warehouse_sales_total)
    return render_template('/accountant/supplier_calculation_daily.html', form=form, suppliers_delivery_total=suppliers_delivery_total, suppliers_warehouse_sales_total=suppliers_warehouse_sales_total)


@accountant_supplier_calculation_blueprint.route('/accountant/supplier/calculation/two-weeks', methods=['GET','POST'])
@login_required
@has_role('accountant')
def accountant_supplier_calculation_two_week():
    ...
    for supplier in suppliers:
        ...
        daily_data = [(
            i.day,
            connection.execute('SELECT COUNT(delivery.id) as total_delivery_count FROM sunsundatabase2.user as supplier join sunsundatabase2.delivery as delivery on supplier.id=delivery.user_id WHERE (delivery.received_from_clerk_date BETWEEN :date1 AND :date2) and supplier.id=:supplier_id and delivery.is_delivered=true GROUP BY supplier.company_name;', {"date1": (i - timedelta(days=1)).replace(hour=17, minute=0), "date2": i.replace(hour=17, minute=0), "supplier_id": supplier.id}).scalar() or 0,
            connection.execute('SELECT SUM(delivery.total_amount) as total_amount FROM sunsundatabase2.user as supplier join sunsundatabase2.delivery as delivery on supplier.id=delivery.user_id WHERE (delivery.received_from_clerk_date BETWEEN :date1 AND :date2) and supplier.id=:supplier_id and delivery.is_delivered=true GROUP BY supplier.company_name;', {"date1": (i - timedelta(days=1)).replace(hour=17, minute=0), "date2": i.replace(hour=17, minute=0), "supplier_id": supplier.id}).scalar() or 0,
            connection.execute('SELECT COUNT(warehouse_sale.id) as total_warehouse_sale_count FROM sunsundatabase2.user as supplier join sunsundatabase2.warehouse_sale as warehouse_sale on supplier.id=warehouse_sale.supplier_id WHERE (warehouse_sale.received_from_clerk_date BETWEEN :date1 AND :date2) and supplier.id=:supplier_id and warehouse_sale.is_completed=true GROUP BY supplier.company_name;', {"date1": (i - timedelta(days=1)).replace(hour=17, minute=0), "date2": i.replace(hour=17, minute=0), "supplier_id": supplier.id}).scalar() or 0,
            connection.execute('SELECT SUM(warehouse_sale.total_amount) as total_amount FROM sunsundatabase2.user as supplier join sunsundatabase2.warehouse_sale as warehouse_sale on supplier.id=warehouse_sale.supplier_id WHERE (warehouse_sale.received_from_clerk_date BETWEEN :date1 AND :date2) and supplier.id=:supplier_id and warehouse_sale.is_completed=true GROUP BY supplier.company_name;', {"date1": (i - timedelta(days=1)).replace(hour=17, minute=0), "date2": i.replace(hour=17, minute=0), "supplier_id": supplier.id}).scalar() or 0,
    ...
    return render_template('/accountant/supplier_calculation_two_week.html', suppliers_datas=suppliers_datas, current_date=selected_date, day_list=days_data, form=form)
...
#                 warehouse_total_amount = connection.execute('SELECT sum(warehouse_sale.total_amount) as total_amount FROM sunsundatabase2.user as supplier join sunsundatabase2.warehouse_sale as warehouse_sale on supplier.id=warehouse_sale.supplier_id where (warehouse_sale.received_from_clerk_date BETWEEN :date1 AND :date2) and supplier.id=:supplier_id and warehouse_sale.is_completed=true group by supplier.company_name;', {"date1": date1.replace(hour=17, minute=0), "date2": date2.replace(hour=17, minute=0), "supplier_id": supplier.id}).scalar()
#                 day_orders = connection.execute('SELECT count(delivery.id) as total_delivery_count FROM sunsundatabase2.user as supplier join sunsundatabase2.delivery as delivery on supplier.id=delivery.user_id where (delivery.received_from_clerk_date BETWEEN :date1 AND :date2) and supplier.id=:supplier_id and delivery.is_delivered=true group by supplier.company_name;', {"date1": date1.replace(hour=17, minute=0), "date2": date2.replace(hour=17, minute=0), "supplier_id": supplier.id}).scalar()
...
#             supplier_format.insert(4, (daily_data))
#             suppliers_datas.append(supplier_format)
#         return render_template('/accountant/supplier_calculation_two_week.html', suppliers_datas=suppliers_datas, current_date=current_date, day_list=days_data, form=form)
